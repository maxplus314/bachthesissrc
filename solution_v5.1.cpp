...
#define $po$ for (int p = 0, pp = 0; auto i: ord)
  ...
  void wtdoag() {
    ...
    relabel(label);
    uint64_t hsh = 0;
    for (auto x: sd) out.wtbi(x, 3), hsh = hsh * 7 + x;
    for (auto x: hm) out.wtbi(x, 4), hsh = hsh * 7 + x;
    out.wbflush();
    out.wti(href.size(), 4);
    for (auto x: href) out.wti(x, pi), hsh = hsh * 7 + x;
  }

  void rddoag() {
    int s1, s2, inlissz, hrs, s3;
    vector<int> sd(nc - contbit.size() * 3), hm(sd.size()), href;
    uint64_t hsh = 0;
    for (auto& x: sd) in.rdbi(x, 3), hsh = hsh * 7 + x;
    for (auto& x: hm) in.rdbi(x, 4), hsh = hsh * 7 + x;
    in.rbflush();
    in.rdi(hrs, 4);
    href.assign(hrs, -1);
    for (auto& x: href) in.rdi(x, pi), hsh = hsh * 7 + x;
    vector<int> roots{};
    for (int i = sd.size(); i--; ) {
  ...
  void wtsoa() {
    ...
    $o$ if (!spec[i]) for (int j = 2; j < bitd[i] / 2; ++j) out.wti(data[i][j]);
  ...
  void rdsoa() {
    ...
    $o$ if (!spec[i]) for (int j = 2; j < bitd[i] / 2; ++j) in.rdi(data[i][j]);
